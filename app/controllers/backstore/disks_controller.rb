class Backstore::DisksController < ApplicationController
  ...
  def index
    ...
    if invalid_date_params?
      errors << "El año de inicio del rango no puede ser mayor al año final. "
    else
  ...
  def create
    ...
    respond_to do |format|
      if @disk.save
        ...
        format.html { redirect_to backstore_disk_path(@disk) }
  ...
  def update
    @genres = Genre.ordered
    if params[:disk][:remove_audio_sample].present?
      if params[:disk][:remove_audio_sample] == "1"
        @disk.remove_audio_sample!
      end
      params[:disk].delete(:remove_audio_sample)
    end
    respond_to do |format|
      if @disk.update(disk_params)
        flash[:notice] = "Disco editado exitosamente"
        format.html { redirect_to backstore_disk_path(@disk), status: :see_other }
  ...
  def destroy
    ActiveRecord::Base.transaction do
      ...
      if @disk.save
        ...
        format.html { redirect_to backstore_disks_path, status: :unprocessable_entity }
        format.json { render json: @disk.errors, status: :unprocessable_entity }
  ...
  private
    ...
    def disk_params
      params.require(:disk).permit(
        :title, :artist, :year, :description, :price, :stock, :format, :state, :cover, :audio_sample, :remove_audio_sample, genre_ids: [], images: []
      )
    end
